...
int main() {
   ...
   {                                                                                //same with j <= userInput.size()
      if (isspace(userInput.at(i)) == 0) //no white space;                          //if both ends letter match, executes; 
      {
         i++;
      }
      ...
      {
      	j--; 
     	//removed a spaceCounter because only one is needed; values iterate both ways so don't want to read ' ' twice. 
	  }	 
	  counter++;
   }
